...
exports.crawl = function(url, maxBuffer, callback) {
	...
	var crawlJsFile = path.join(__dirname, 'phantomjs-scripts/crawl.js');
	...
	if (process.env.REDISCLOUD_URL) { 
		...
		childArgs = [binPath, crawlJsFile, url];
		...
		childProcess.exec('xvfb-run -s "-ac -screen 0 1280x1024x24 +render -noreset" ' + childArgs[0] + ' ' + childArgs[1] + ' ' + childArgs[2], { maxBuffer: maxBuffer }, function(err, stdout, stderr) {
		if (err) console.log(err);
			callback(err, stdout);
		});
	} else {
